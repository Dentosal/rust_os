...
use super::process::{Process, ProcessResult, Status};
use super::ProcessId;
...
impl State {
    ...
    pub fn process_by_id(&mut self, pid: ProcessId) -> Option<&Process> {
    ...
    pub fn process_by_id_mut(&mut self, pid: ProcessId) -> Option<&mut Process> {
        for p in self.process_list.iter_mut() {
            if p.id() == pid {
                return Some(p);
            }
        }
    ...
    unsafe fn create_process(&mut self, parent: Option<ProcessId>, elf: ElfImage) -> ProcessId {
        ...
        let process = memory::configure(|mm| {
    ...
    pub fn terminate(&mut self, target: ProcessId, status: ProcessResult) -> bool {
