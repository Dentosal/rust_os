...
use crate::driver::pci::{self, Device};
use crate::memory::{self, dma_allocator::DMARegion, MemoryController};

use super::{notify_irq, Packet, NIC};
...
impl RTL8139 {
    pub fn try_new() -> Option<Box<dyn NIC>> {
        ...
        let device = pci_ctrl.find(|dev| {
            //	RTL-8139
            (dev.vendor, dev.id) == (0x10ec, 0x8139) &&
            // Ethernet controller
            dev.class.0 == 2 && dev.class.1 == 0
        })?;

        Some(Box::new(RTL8139 {
            pci_device: device,
            irq: device.get_interrupt_line().expect("Missing interrupt line"),
            io_base: 0,
            buffers: None,
            link_up: false,
        }))
    ...
    fn reset(&mut self) {
        ...
        unsafe {
            ...
            // Set up rx buffer
            r_rxbuf.write(buffers.rx.start_addr_u32());
            ...
            for (i, region) in buffers.tx.iter().enumerate() {
                let mut r_txaddr0 =
                    UnsafePort::<u32>::new(self.io_base + reg::TXADDR0 + (i as u16 * 4));
                r_txaddr0.write(region.start_addr_u32());
            }
    ...
    fn receive(&mut self) -> Option<Packet> {
        ...
        unsafe {
            let rx_ptr: *const u8 = buffers.rx.virt_addr().as_ptr();
            let packet_start = rx_ptr.add(buffers.rx_offset);
            ...
            let nic_rx_offset =
                (((buffers.rx_offset as u16) + length + 4 + 3) & !3) % (RX_BUFFER_SIZE as u16);
            let mut r_capr: UnsafePort<u16> = UnsafePort::new(self.io_base + reg::CAPR);
            r_capr.write((buffers.rx_offset - 0x10) as u16);
            buffers.rx_offset = (nic_rx_offset % (RX_BUFFER_SIZE as u16)) as usize;

            // Return packet
            Some(Packet(packet_buffer))
        }
    }
    ...
    fn select_buffer(&self) -> Option<usize> {
        for i in 0..TX_BUFFER_COUNT {
            ...
            let potential_buffer = (buffers.tx_next + i) % 4;
...
impl NIC for RTL8139 {
    ...
    fn init(&mut self) -> bool {
        use crate::interrupt::FREE_IRQ_HOOK;

        // Bind interrupt handler
        let mut irqhook = FREE_IRQ_HOOK.try_lock().unwrap();
        irqhook.set_by_irq(self.irq, notify_irq);

        unsafe {
            self.pci_device.enable_bus_mastering();
        }

        self.io_base = (self.pci_device.get_bar(0) & (!1u32)) as u16;

        self.buffers = Some(memory::configure(|mem_ctrl: &mut MemoryController| {
            Buffers {
                rx: mem_ctrl
                    .dma_allocator
                    .allocate(RX_BUFFER_SIZE + PACKET_SIZE_MAX),
                tx: (0..TX_BUFFER_COUNT)
                    .map(|_| mem_ctrl.dma_allocator.allocate(TX_BUFFER_SIZE))
                    .collect::<Vec<_>>(),
                tx_next: 0,
                rx_offset: 0,
            }
    ...
    fn send(&mut self, packet: &[u8]) {
        ...
        let dst = buffers.tx[buffer_index].virt_addr().as_mut_ptr();
        unsafe {
